...
int main (void) {
    ...
    // needed to read loop
    WORD key = 0;
    __int64 current_sector = 0;
    __int64 total_sectors = 0;


    // allocate memory for AHCI data buffer
    DiskMultiSectorTransferBuffer = (BYTE *) malloc(MAX_SECTOR_SIZE * MULTI_SECTOR);
    if (DiskMultiSectorTransferBuffer == NULL) {
        printf("Could not allocate memory for transfer buffer!\n");
        exit(0);
    }
    memset(DiskMultiSectorTransferBuffer, 0, MAX_SECTOR_SIZE * MULTI_SECTOR);
    ...
    // wait for esc to exit the program, start reading from disk 0 - first in drives array
	status = TRUE;
	total_sectors = diskdrive[0].total_sectors;
    while (!key && current_sector < total_sectors) {
        if (kbhit()) key = getch();
        else key = 0;
        if (key == 27) break;
		
        // read from sector 0 to the end
        printf("Reading sector : %d\n", current_sector);
        status = ahci_read_multisector (current_sector, &diskdrive[0]);
        if (!status) {
			printf ("Error at : %d", current_sector);
			break;
		}
        current_sector += MULTI_SECTOR;
		cursor_up();
    }
    ...
    ahci_close_ahci ();
    free(DiskMultiSectorTransferBuffer);
