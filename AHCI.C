...
// pointer to AHCI data transfer buffer
...
// reads multiple sectors into previously allocated memory pointed to by DiskMultiSectorTransferBuffer
// the number of sectors read is equal to MULTI_SECTOR defined in drives.h
// the number of bytes read is equal to MULTI_SECTOR * sdrive->bytes_per_sector
BOOL ahci_read_multisector (__int64 starting_sector, DISKDRIVE *sdrive)
{
    ...
    int count, bytes_to_transfer, i;
    WORD *diskbuffer;

    // number of sectors (MULTI_SECTOR = 256 by default) - check end of drive
    if ((starting_sector + MULTI_SECTOR) > (sdrive->total_sectors - 1)) {
        count = sdrive->total_sectors - 1 - starting_sector;
    } else {
        count = MULTI_SECTOR;
    }
    bytes_to_transfer = count * sdrive->bytes_per_sector;
...
// Bit 0: ERR (Error) Set when an error occurred during the previous ATA command.
    ...
    // pointer to area that will store the read data for direct ATA (AHCI) version
    diskbuffer = (WORD *)(DiskMultiSectorTransferBuffer);
    ...
    // ahci dma
    // memory for cmd header is already allocated
    cmd_hdr = (HBA_CMD_HEADER *)ahci_pci.ports[sdrive->ahci_port].clb;
    cmd_hdr->cfl = sizeof(FIS_REG_H2D)/sizeof(DWORD);       // Command FIS size = 5
    cmd_hdr->a = 0;
    cmd_hdr->w = 0;                                         // read from device
    cmd_hdr->p = 0;
    cmd_hdr->prdtl = (WORD)(((count - 1) * sdrive->bytes_per_sector) >> 13) + 1;   // / 8192 - 8KB PRDT entries
    cmd_hdr->prdbc = 0;                                     // Physical region descriptor byte count transferred - set to 0 on start
    cmd_hdr->ctba = ahci_pci.ports[sdrive->ahci_port].cmdtbl;          // Command table base address

    // fill in cmdl (cmdtbl) - HBA_CMD_TBL core size is 0x80 + PRDT entries
    cmd_tbl = (HBA_CMD_TBL *)ahci_pci.ports[sdrive->ahci_port].cmdtbl;
    memset (cmd_tbl, 0, 0x80 + (cmd_hdr->prdtl * sizeof(HBA_PRDT_ENTRY)));

    // 8K bytes (16 sectors) per PRDT + last PRDT with remaining sectors
    for (i = 0; i < cmd_hdr->prdtl - 1; i++) {
        cmd_tbl->prdt_entry[i].dba = (DWORD)diskbuffer;
        cmd_tbl->prdt_entry[i].dbc = 8192 - 1;          // 8K bytes
        cmd_tbl->prdt_entry[i].i = 0;
        diskbuffer += 4096;                         // 4K words
        bytes_to_transfer -= 8192;                  // 8K for each full PRDT entry
    }
    cmd_tbl->prdt_entry[i].dba = (DWORD)diskbuffer;
    cmd_tbl->prdt_entry[i].dbc = bytes_to_transfer - 1; // remaining bytes to transfer
    cmd_tbl->prdt_entry[i].i = 1;

    // command
    fis = (FIS_REG_H2D *)ahci_pci.ports[sdrive->ahci_port].cmdtbl;
    fis->fis_type = FIS_TYPE_REG_H2D;
    fis->c = 1;                                     // Write command register

    // test for 48bit available flag
    if (sdrive->lba_48bit) {
        fis->command = ATA_CMD_READ_DMA_EX;         // ATA command 48-bit LBA
        fis->lba0 = (BYTE) starting_sector;
        fis->lba1 = (BYTE) (starting_sector >> 8);
        fis->lba2 = (BYTE) (starting_sector >> 16);
        fis->device = 1 << 6;                       // LBA mode = 0x40
 
        fis->lba3 = (BYTE) (starting_sector >> 24);
        fis->lba4 = (BYTE) (starting_sector >> 32);
        fis->lba5 = (BYTE) (starting_sector >> 40);
 
        fis->countl = LOBYTE(count);
        fis->counth = HIBYTE(count);
    } else {
        fis->command = ATA_CMD_READ_DMA;             // ATA command 28-bit LBA
        fis->lba0 = (BYTE) starting_sector;
        fis->lba1 = (BYTE) (starting_sector >> 8);
        fis->lba2 = (BYTE) (starting_sector >> 16);
        fis->device = 1 << 6;                        // LBA mode = 0x40
 
        fis->lba3 = (BYTE) ((starting_sector >> 24) & 0x0f);
        fis->lba4 = (BYTE) 0;
        fis->lba5 = (BYTE) 0;
    ...
    // adjust received FIS pointer
    hba_fis = (HBA_FIS *)ahci_pci.ports[sdrive->ahci_port].fb;

    // clear interrupt status
    intstatus = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_IS);
    if (intstatus) ahci_port_write_dword (sdrive->ahci_port, AHCI_REG_PORT_IS, intstatus);

    // ahci command execute
    ahci_port_write_dword (sdrive->ahci_port, AHCI_REG_PORT_CI, 1);                  // port x command issue

    // timeout defined in drev.h
    QueryPerformanceCounter (&WaitStart);
    while (1) {
        intstatus = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_IS);
        if (intstatus) {
            // no error intstatus = 33 (DPS + DHRS), error intstatus = 0x40000021 (TFES + DPS + DHRS)
            // clear port interrupt status
            ahci_port_write_dword (sdrive->ahci_port, AHCI_REG_PORT_IS, intstatus);
            if (intstatus & AHCI_REG_PORT_IS_TFES) {
                // Task File Error Status
                status = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_TFD) & 0xFF;
                error = (ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_TFD) >> 8) & 0xFF;
                break;
            }
            if (intstatus & AHCI_REG_PORT_IS_DHRS) {
                // DHRS setup FIS interrupt - bit0 - set even on transfer error.
                // status = rfis->rfis.status;
                // error = rfis->rfis.error;
                status = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_TFD) & 0xFF;
                error = (ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_TFD) >> 8) & 0xFF;
                break;
            }
            if (intstatus & AHCI_REG_PORT_IS_PSS) {
                // PSS - PIO setup FIS interrupt - bit1
                // A PIO setup FIS has been received with the I bit set. It has been copied into system memory
                // and the data related to that FIS has been transfered.
                // status = rfis->psfis.status;
                // error = rfis->psfis.error;
                status = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_TFD) & 0xFF;
                error = (ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_TFD) >> 8) & 0xFF;
                break;
            }
        ...
        WaitTime = (DWORD) ((unsigned __int64)(1000 * (WaitEnd - WaitStart)/Frequency));        // calculate wait time in ms
        if (WaitTime > TIMEOUT) {
            // printf("HBA : Device command request taking too long!\n");
            status = 0xFF;
            error = 0xFF;
            break;
        }
    ...
    // clear global interrupt bit in AHCI_REG_IS for the corresponding port
    ahci_global_write_dword (AHCI_REG_IS, 1 << sdrive->ahci_port); // port x command issue
    ...
    // return true if success
    if (((status & (ATA_BUSY | ATA_DF | ATA_ERR)) == 0) && (status & ATA_DRDY)) {
        return TRUE;
    }

    // clear start port register
    cmd = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_CMD);
    cmd &= ~(AHCI_REG_PORT_CMD_ST);
    ahci_port_write_dword (sdrive->ahci_port, AHCI_REG_PORT_CMD, cmd);
    ...
    // clear error bits in AHCI_REG_PORT_SERR to enable capturing of new errors
    val = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_SERR);
    ahci_port_write_dword (sdrive->ahci_port, AHCI_REG_PORT_SERR, val);
    ...
    // issue a COMRESET to the device to put it in an idle state if
    // status.BSY and status.DRQ is set to 1
    tfd = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_TFD);
    // test ATA flags
    if (tfd & (ATA_BUSY | ATA_DRQ)) {
        // issuing a COMRESET to stop the device - AHCI_REG_PORT_SCTL
        val = ahci_port_read_dword (sdrive->ahci_port, AHCI_REG_PORT_SCTL);
        // set Device Detection Initialization to 1 for 1ms
        ahci_port_write_dword (sdrive->ahci_port, AHCI_REG_PORT_SCTL, val | 1);
        delay(1);
        ahci_port_write_dword (sdrive->ahci_port, AHCI_REG_PORT_SCTL, val);
    }
